...
const ResultsPage: React.FC<ResultsPageProps> = ({ searchData, onBack, onSelectTrip }) => {
  const trips: Trip[] = [
    {
      id: '1',
      company: 'SOTRAL',
    ...
    },
    {
      ...
      totalSeats: 45,
      rating: 4.8,
      amenities: ['WiFi', 'Climatisation', 'Collation', 'USB']
    },
    {
      id: '3',
      company: 'AICE Transport',
      departure: searchData.departure,
      arrival: searchData.arrival,
      departureTime: '14:00',
      arrivalTime: '18:15',
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <div className="text-center">
                <div className="text-lg font-semibold text-gray-900">{searchData.departure}</div>
                <div className="text-sm text-gray-500">Départ</div>
              </div>
              <ArrowRight className="w-5 h-5 text-gray-400" />
              <div className="text-center">
                <div className="text-lg font-semibold text-gray-900">{searchData.arrival}</div>
                <div className="text-sm text-gray-500">Arrivée</div>
              </div>
            </div>
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-900">{formatDate(searchData.date)}</div>
              <div className="text-sm text-gray-500">{trips.length} trajets trouvés</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          ...
          <select className="px-4 py-2 border border-gray-300 rounded-lg bg-white">
            ...
            <option>SOTRAL</option>
            <option>Trans-Sahel</option>
            <option>AICE Transport</option>
          </select>
        </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                  ...
                  <div className="flex items-start space-x-4">
                    <div className="w-16 h-16 bg-blue-100 rounded-lg flex items-center justify-center">
                      <span className="text-blue-600 font-bold text-sm">{trip.company.substring(0, 2)}</span>
                    </div>
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">{trip.company}</h3>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-sm text-gray-500">{trip.arrival}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-6">
                    <div className="text-center">
                      <div className="flex items-center text-gray-600 mb-1">
                        ...
                        <span className="text-sm">{trip.availableSeats} places</span>
                      </div>
                      <div className="text-xs text-gray-500">sur {trip.totalSeats}</div>
                    </div>
