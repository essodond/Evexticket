      ...
      <CompanyPageShell title={agency?.nom || 'Détail de l’agence'} description="Coordonnées, guichets, personnel et activité du point de vente." actions={agency && <button type="button" onClick={() => setShowEdit(true)} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50"><Edit3 className="h-4 w-4" /> Modifier</button>}>
        ...
        {agency && <>
          ...
          <section className="grid gap-6 lg:grid-cols-2">
            <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Coordonnées</h2><dl className="mt-5 space-y-4 text-sm"><div><dt className="text-slate-400">Adresse</dt><dd className="mt-1 font-medium text-slate-800">{agency.adresse}</dd></div><div><dt className="text-slate-400">Téléphone</dt><dd className="mt-1 font-medium text-slate-800">{agency.telephone}</dd></div><div><dt className="text-slate-400">Position de la gare</dt><dd className="mt-1 font-medium text-slate-800">{agency.latitude && agency.longitude ? `${agency.latitude}, ${agency.longitude}` : 'Non renseignée'}</dd></div><div><dt className="text-slate-400">Statut</dt><dd className="mt-1"><StatusBadge active={agency.is_active} /></dd></div></dl></article>
